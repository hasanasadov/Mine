...
//---------getchar--------int getchar(void) -- read the ne character
...
int main()
{
    ...
    sscanf(mystr, "%d %lf", &intNumber, &doubleNumber);

    printf("my integer = %d\n", intNumber);
    printf("my Doublefloat = %lf", doubleNumber);
}
*/
...
int main () {
   ...
   if(memcpy(c,d, 2*sizeof(short))==0){
      printf("\nsame array\n");
   }
   else{
      printf("\nnot same array\n");
...
int main () {
   ...
   int  a[2]={2,3}; //0 0 0 2 0 0 0 4
   int  b[2]={1,5};

   if(memchr(a,0, 2*sizeof(int))!=NULL){
      printf("\nFound the Byte\n");
   }
   else{
      printf("\nNOT Found the Byte\n");
